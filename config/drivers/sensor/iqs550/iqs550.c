...
static int iqs5xx_end_comms(const struct device *dev) {
    ...
    return iqs5xx_write_reg16(dev, IQS5XX_REG_END_COMMS, &end_byte, 1);
}
...
static void iqs5xx_work_handler(struct k_work *work) {
    ...
    LOG_DBG("Raw Rel: X=%d, Y=%d | Abs: X=%u, Y=%u | Fingers: %u | Strength: %u | Sys0: 0x%02x | Gest0: 0x%02x | Gest1: 0x%02x",
...
static int iqs5xx_init(const struct device *dev) {
    ...
    // QMK just reads a register, which implicitly wakes
    ...
    // QMK uses 10ms (100Hz). Use Kconfig default or fixed value?
    ...
    if (ret == 0) {
        ...
        // QMK enables others too, check if needed
    ...
    k_sleep(K_MSEC(AZOTEQ_IQS5XX_REPORT_RATE + 5)); // Wait for first report cycle

    LOG_INF("Initialized IQS%u touchpad", data->product_number);
    return 0;
}
